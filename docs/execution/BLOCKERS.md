...
| BLK-M0-05 | M0-05, M1-06 | AWS credentials and a nonempty `AWS_DEFAULT_REGION` are present locally (values are not logged), but approved account/domain/budget/operator bindings and deployment secret references are not in repository configuration; `make infra-validate` intentionally returns BLOCKED | Deployment owner supplies nonsecret bindings and approves Terraform plan/cost before any apply | OPEN |
